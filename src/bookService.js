...
addBook(new Book(2,'¿Sueñan los androides con ovejas eléctricas?','Ficción distópica',1968,1,'La acción se sitúa en un mundo lleno de polvo radiactivo después de una guerra nuclear que terminó matando a la mayoría de los animales, llevando a que la gente tenga animales eléctricos.'))
addBook(new Book(3,'1984','Derechos humanos',1949,4,'El personaje principal de la novela es Winston Smith, que trabaja en el Ministerio de la Verdad. Su cometido es reescribir la historia'))
addBook(new Book(4,'El necronomicón','Terror',2008,3,'El Necronomicón es descrito como un libro de saberes arcanos y magia ritual, cuya lectura provoca la locura y la muerte.'))
...
export function deleteElement(idDeleteBook) {
...
function addBook(NewBk) {
    books.set(NewBk.id, NewBk);
    addBookToDOM(NewBk, books.length - 1);
    showHide('Anadir');
    showHide('content');
    showHide('NL');
}
...
function newBook() {
    ...
    let l = new Book(id, title,author,year,copies, description);
    addBook(l);
}
